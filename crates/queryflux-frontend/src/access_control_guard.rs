        ...
        if check_write {
            for write_op in &write_ops {
                ...
                let filters: Vec<String> = decision
                    ...
                    .collect();
                if !filters.is_empty() {
                    // UPDATE/DELETE are scoped by ANDing the filter into their WHERE. Every
                    // other write (INSERT, MERGE, TRUNCATE, DDL) has no WHERE limiting which
                        ...
                        return GuardResult::deny(
                            format!(
                                "access control: the policy returned row filters for {}, which \
                                 are only supported for table.select, table.update and \
                                 table.delete",
                                write_op.as_str()
                            ),
                            "ACCESS_REWRITE_UNSUPPORTED_FOR_WRITE",
