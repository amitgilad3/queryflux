    ...
    async fn check(&self, ctx: &GuardContext<'_>) -> GuardResult {
        ...
        if check_write {
            ...
            let filters: Vec<String> = decision
                ...
                .collect();
            if !filters.is_empty() {
                // UPDATE/DELETE are scoped by ANDing the filter into their WHERE. INSERT,
                // MERGE and TRUNCATE have no WHERE limiting which rows the write touches, so
                // deny rather than silently skip a restriction the policy meant to apply.
                if matches!(operation.as_str(), "table.update" | "table.delete") {
                    write_scope = Some((targets[0].table.clone(), filters));
                } else {
                    return GuardResult::deny(
                        format!(
                            "access control: the policy returned row filters for {}, which are \
                             only supported for table.select, table.update and table.delete",
                            operation.as_str()
                        ),
                        "ACCESS_REWRITE_UNSUPPORTED_FOR_WRITE",
                    );
