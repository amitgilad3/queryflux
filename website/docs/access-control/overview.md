...
Every table a statement **reads** is evaluated as `table.select` — deny, row filter, column mask — including reads *inside* a write: `INSERT … SELECT`, `CREATE TABLE … AS SELECT`, `CREATE VIEW … AS SELECT`, `MERGE … USING`, and subqueries in `UPDATE`/`DELETE`. A protected table cannot be copied out through a write statement.
